...
//if($_SERVER["REQUEST_METHOD"]=="POST"){
  ...
  if($rows >= 1){
     ...
     $something = "You Have Not Made Any Postings Yet!";
   }
...
<h5><a href = "AddBook.html">Post A Book For Sale</a></h5>
<h5><a href = "EditAccount">Edit Account Info</a></h5>
...
<?php
  ...
  for($j = 1; $j<=$i; $j++){
    ...
    echo "<form align='right' action='EditPost' method='POST'>
          <input type='hidden' name='act' value ='postEdit'/>
          <input type='hidden' name='id' value ='".$PostArray[$j]['id']."'/>
          <input type='Submit'  value='Edit'>
          </form>";

    echo "<form align='right' action='' method='POST'>
          <input type='hidden' name='id' value ='".$PostArray[$j]['id']."'/>
          <input type='Submit'  value='Details'>
          </form>";

    echo "<form align='right' action='EditPost' method='POST'>
          <input type='hidden' name='act' value ='Sold'/>
          <input type='hidden' name='id' value ='".$PostArray[$j]['id']."'/>
          <input type='hidden' name='statusOfBook' value ='".$PostArray[$j]['statusOfBook']."'/>
          <input type='Submit'  value='Mark As Sold'>
          </form>";
  }
  //Displays no postings if there are none
  if($ifNoCurrentPostings == 0){
    echo "<h4>You Have No Current Postings For Sale!</h4>";
  }
  ...
  for($y = 1; $y<$opa+1; $y++){
    ...
    echo "<form align='right' action='EditPost' method='POST'>
          <input type='hidden' name='act' value ='postEdit'/>
          <input type='hidden' name='id' value ='".$PostArray[$oldPostArray[$y]]['id']."'/>
          <input type='Submit'  value='Edit'>
          </form>";
    ...
    echo "<form align='right' action='EditPost' method='POST'>
          <input type='hidden' name='act' value ='Sold'/>
          <input type='hidden' name='id' value ='".$PostArray[$oldPostArray[$y]]['id']."'/>
          <input type='hidden' name='statusOfBook' value ='".$PostArray[$oldPostArray[$y]]['statusOfBook']."'/>
